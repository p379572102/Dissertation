...
traffic_2018.minor <- st_buffer(traffic_2018.minor, 10)
traffic_2018.minor <- st_transform(traffic_2018.minor, 4326)

traffic_2018.minor <- st_join(traffic_2018.minor,graphs)
summary(is.na(traffic_2018.minor))# Check if the traffic counts number is right

traffic_2018.minor <- traffic_2018.minor[,c("way_id", "aadt", "centrality", 
                                            "nearst_junc_dist", "major_aadt")]
...
traffic_minor %>% ggplot(aes(x = aadt, y = nearst_junc_dist)) +
  geom_point(aes(color = highway)) +
  geom_smooth(method="lm", se=F, fullrange=FALSE, level=0.95)
...
traffic_minor <- st_drop_geometry(traffic_minor)

cormat <- cor(traffic_minor, use ="complete.obs", method = "pearson")
cor(traffic_minor$centrality, traffic_minor$aadt)
cor(traffic_minor$major_aadt, traffic_minor$aadt)

# simple model
m1 <- lm(aadt ~ centrality + major_aadt + nearst_junc_dist, data = traffic_minor)
summary(m1)
