...
### Find the 20 of nearest neighbour centroids , and find potentially duplicated ones
nn = RANN::nn2(lines_major_cents, k = 50) ### k value depends on the network###

for(k in 1:2){
  for(i in 1:nrow(lines_major)){
    if(is.na(lines_major$ref[i])){
      for(j in 2:50){ #j=1 is the point itself
        idx <- nn$nn.idx[i,j]
