...
distance_junction <- list()
for(i in 1:ncol(dists)){
  sub <- dists[,i]
  sub <- sub[!is.na(sub)]
  if(length(sub) == 0){
    nearst_junction[[i]] <- NA
    distance_junction[[i]] <- NA
  } else {
    ...
    nearst_junction[[i]] <- min_name
    distance_junction[[i]] <- min_dist
  }
  
}
nearst_junction <- unlist(nearst_junction)
distance_junction <- unlist(distance_junction)

lines_minor$nearst_junc_dist <- distance_junction
